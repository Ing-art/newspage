      ...
      <article class="card bg-dark text-white zoom overflow-hidden rounded" style="height:500px width:auto">
        ...
        <div class="card-img-overlay d-flex flex-column justify-content-end bg-lg-shadow">
          <div class="w-100 p-3 m-0">
            ...
            <a href="{{route('articles.show', $toparticle->id)}}" class="text-decoration-none text-light">
              <h3 class="card-title">{{$toparticle->headline}}</h3>
            </a>
            <div>
              <span class="card-text d-none d-md-block">by {{$toparticle->user->name}} |
                {{\Carbon\carbon::parse($toparticle->published_at)->format('jS F Y')}}</span>
              <small class="text-white d-none d-md-block">
                <i class="fas fa-eye"></i> {{$toparticle->visits}}
              </small>
              {{-- <span class="card-text d-none d-md-block">Published on
                {{\Carbon\carbon::parse($toparticle->published_at)->format('jS F Y')}}</span> --}}
            </div>
          </div>
        </div>
